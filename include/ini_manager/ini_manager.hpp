...
public:
  ...
  auto get_value(section section, key key) const noexcept
  ...
  auto get_value_or_default(section section,
                            key key,
  ...
  auto get_value_or_default(section section,
                            key key,
                            T default_value) const noexcept -> T
  {
  ...
  void set_value(std::string_view section,
                 std::string_view key,
  ...
  void set_value(std::string_view section,
                 ...
                 bool value) noexcept
  {
    (*m_data)[std::string {section}][std::string {key}] =
        value ? "true" : "false";
  }
  ...
  auto load_file(const std::string& file_path)
  ...
  auto write_file() const -> std::expected<void, std::error_code>
  {
    if (m_file_path.empty()) {
      return std::unexpected(
          std::error_code(static_cast<int>(std::errc::invalid_argument),
...
private:
  ...
  auto load(const std::string& file_path)
      -> std::expected<void, std::error_code>
  {
  ...
  auto write(std::ostream& ostream) const
      -> std::expected<void, std::error_code>
  {
