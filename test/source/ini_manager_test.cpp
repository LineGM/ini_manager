        ...
        describe("ini::ini::ini_manager") = [] {
            ...
            describe("from_stream") = [] {
                it("should parse valid INI data from a stream") = [] {
                    ...
                    expect(manager.get_value(ini::section{"section1"}, ini::key{"key1"}) == "value1");
                    expect(manager.get_value(ini::section{"section1"}, ini::key{"key2"}) == "value2");
                    expect(manager.get_value(ini::section{"section2"}, ini::key{"key3"}) == "value3");
                    expect(!manager.get_value(ini::section{"nonexistent"}, ini::key{"key"}));
                };
                ...
                it("should handle streams with invalid syntax (no equals sign)") = [] {
                    ...
                    const auto &manager = result.value();
                    expect(!manager.get_value(ini::section{"section"}, ini::key{"invalid_line"}));
                };
            };
            ...
            describe("operator(non-const)") = [] {
                ...
                it("should create new sections and keys if they don't exist") = [] {
                    ...
                    expect(manager.get_value(ini::section{"new_section"}, ini::key{"new_key"}) == "new_value");
                };
            };
            ...
            describe("operator(const)") = [] {
                it("should allow read-only access to existing values") = [] {
                    ...
                    expect(const_manager["section"]["key"].value() == "test_value");
                };
            ...
            describe("get_value (string)") = [] {
                given("an ini::ini_manager with a string value") = [] {
                    ...
                    it("should return the value as an optional string") = [&] {
                        expect(manager.get_value(ini::section{"section"}, ini::key{"key"}) == "string_value");
                    };
                };
                ...
                given("an ini::ini_manager with no such key") = [] {
                    ...
                    it("should return an empty optional") = [&] {
                        expect(!manager.get_value(ini::section{"section"}, ini::key{"nonexistent_key"}));
                    };
                };
                ...
                given("an ini::ini_manager with no such section") = [] {
                    ini::ini_manager manager;
                    it("should return an empty optional") = [&] {
                        expect(!manager.get_value(ini::section{"nonexistent_section"}, ini::key{"key"}));
                    };
                };
            };
            ...
            describe("get_value (templated)") = [] {
                given("an ini::ini_manager with an integer value") = [] {
                    ...
                    it("should return the value as an optional integer") = [&] {
                        expect(manager.get_value<int>(ini::section{"section"}, ini::key{"int_key"}) == 123);
                    };
                };
                ...
                given("an ini::ini_manager with a double value") = [] {
                    ...
                    it("should return the value as an optional double") = [&] {
                        expect(manager.get_value<double>(ini::section{"section"}, ini::key{"double_key"}) == 3.14);
                    };
                };
                ...
                given("an ini::ini_manager with a boolean value (true)") = [] {
                    ...
                    it("should return the value as an optional boolean (true)") = [&] {
                        expect(manager.get_value<bool>(ini::section{"section"}, ini::key{"bool_key_true"}) == true);
                        expect(manager.get_value<bool>(ini::section{"section"}, ini::key{"bool_key_1"}) == true);
                        expect(manager.get_value<bool>(ini::section{"section"}, ini::key{"bool_key_True"}) == true);
                    };
                };
                ...
                given("an ini::ini_manager with a boolean value (false)") = [] {
                    ...
                    it("should return the value as an optional boolean (false)") = [&] {
                        expect(manager.get_value<bool>(ini::section{"section"}, ini::key{"bool_key_false"}) == false);
                        expect(manager.get_value<bool>(ini::section{"section"}, ini::key{"bool_key_0"}) == false);
                        expect(manager.get_value<bool>(ini::section{"section"}, ini::key{"bool_key_False"}) == false);
                    };
                };
                ...
                given("an ini::ini_manager with an invalid boolean value") = [] {
                    ...
                    it("should return an empty optional for invalid boolean") = [&] {
                        expect(!manager.get_value<bool>(ini::section{"section"}, ini::key{"bool_key_invalid"}));
                    };
                };
                ...
                given("an ini::ini_manager with a non-numeric string for numeric types") = [] {
                    ...
                    it("should return an empty optional for invalid numeric conversions") = [&] {
                        expect(!manager.get_value<int>(ini::section{"section"}, ini::key{"invalid_int"}));
                        expect(!manager.get_value<double>(ini::section{"section"}, ini::key{"invalid_double"}));
                    };
                };
            };
            ...
            describe("get_value_or_default (string)") = [] {
                given("an ini::ini_manager with a value") = [] {
                    ...
                    it("should return the value") = [&] {
                        expect(manager.get_value_or_default<std::string>(ini::section{"section"}, ini::key{"key"}, "default") == "value");
                    };
                };
                ...
                given("an ini::ini_manager without the value") = [] {
                    ...
                    it("should return the default value") = [&] {
                        expect(manager.get_value_or_default<std::string>(ini::section{"section"}, ini::key{"nonexistent_key"}, "default") == "default");
                    };
                };
            };
            ...
            describe("get_value_or_default (templated)") = [] {
                given("an ini::ini_manager with an integer value") = [] {
                    ...
                    manager.set_value("section", "int_key", "123");
                    it("should return the value") = [&] {
                        expect(manager.get_value_or_default(ini::section{"section"}, ini::key{"int_key"}, 456) == 123);
                    };
                };
                ...
                given("an ini::ini_manager without the value") = [] {
                    ini::ini_manager manager;
                    it("should return the default value") = [&] {
                        expect(manager.get_value_or_default(ini::section{"section"}, ini::key{"nonexistent_key"}, 456) == 456);
                    };
                };
                ...
                given("an ini::ini_manager with an invalid value for the type") = [] {
                    ...
                    manager.set_value("section", "invalid_int", "abc");
                    it("should return the default value") = [&] {
                        expect(manager.get_value_or_default(ini::section{"section"}, ini::key{"invalid_int"}, 456) == 456);
                    };
                };
            };
            ...
            describe("set_value") = [] {
                it("should set a string value") = [] {
                    ini::ini_manager manager;
                    manager.set_value("section", "key", "value");
                    expect(manager.get_value(ini::section{"section"}, ini::key{"key"}) == "value");
                };

                it("should set an integer value") = [] {
                    ini::ini_manager manager;
                    manager.set_value("section", "int_key", 123);
                    expect(manager.get_value(ini::section{"section"}, ini::key{"int_key"}) == "123");
                };

                it("should set a double value") = [] {
                    ini::ini_manager manager;
                    manager.set_value("section", "double_key", 3.14);
                    expect(manager.get_value(ini::section{"section"}, ini::key{"double_key"}) == "3.14");
                };

                it("should create the section if it doesn't exist") = [] {
                    ini::ini_manager manager;
                    manager.set_value("new_section", "key", "value");
                    expect(manager.get_value(ini::section{"new_section"}, ini::key{"key"}) == "value");
                };
                ...
                it("should update the value if the key already exists") = [] {
                    ...
                    expect(manager.get_value(ini::section{"section"}, ini::key{"key"}) == "new_value");
                };
            };
            ...
            describe("set_section") = [] {
                it("should create a new section if it doesn't exist") = [] {
                    ...
                    expect(manager["new_section"].operator[]("some_key").empty()); // Check if section exists
                };
                ...
                it("should not overwrite an existing section") = [] {
                    ...
                    expect(manager.get_value(ini::section{"existing_section"}, ini::key{"key"}) == "value");
                };
            };
            ...
            describe("remove_value") = [] {
                given("an ini::ini_manager with a value") = [] {
                    ...
                    it("should remove the value and return true") = [&] {
                        expect(manager.remove_value(ini::section{"section"}, ini::key{"key"}));
                ...
                given("an ini::ini_manager without the value") = [] {
                    ...
                    it("should return false") = [&] {
                        expect(!manager.remove_value(ini::section{"section"}, ini::key{"nonexistent_key"}));
                    };
                };
                ...
                given("an ini::ini_manager without the section") = [] {
                    ini::ini_manager manager;
                    it("should return false") = [&] {
                        expect(!manager.remove_value(ini::section{"nonexistent_section"}, ini::key{"key"}));
                    };
                };
            };
            ...
            describe("remove_section") = [] {
                ...
                given("an ini::ini_manager without the section") = [] {
                    ini::ini_manager manager;
                    it("should return false") = [&] {
                        expect(!manager.remove_section(ini::section{"nonexistent_section"}));
                    };
                };
            };
            ...
            describe("load_stream") = [] {
                it("should clear existing data and load from a stream") = [] {
                    ini::ini_manager manager;
                    manager.set_value("existing_section", "existing_key", "existing_value");
                    std::stringstream sstream;
                    sstream << "[new_section]\n";
                    sstream << "new_key = new_value\n";
                    auto result = manager.load_stream(sstream);
                    expect(result.has_value());
                    expect(!manager.get_value(ini::section{"existing_section"}, ini::key{"existing_key"}));
                    expect(manager.get_value(ini::section{"new_section"}, ini::key{"new_key"}) == "new_value");
                };
            };
            ...
            describe("add_from_stream") = [] {
                it("should add data from a stream to existing data") = [] {
                    ...
                    expect(result.has_value());
                    expect(manager.get_value(ini::section{"existing_section"}, ini::key{"existing_key"}) == "existing_value");
                    expect(manager.get_value(ini::section{"new_section"}, ini::key{"new_key"}) == "new_value");
                };
            ...
            describe("operator<< (ostream)") = [] {
                it("should write the INI data to the output stream") = [] {
                    ...
                    std::stringstream sstream;
                    sstream << manager;
                    const std::string expected = "[section1]\nkey1 = value1\n\n[section2]\nkey2 = value2\n\n";
            ...
            describe("operator>> (istream)") = [] {
                it("should parse INI data from the input stream") = [] {
                    ...
                    sstream >> manager;
                    expect(manager.get_value(ini::section{"section1"}, ini::key{"key1"}) == "value1");
                    expect(manager.get_value(ini::section{"section2"}, ini::key{"key2"}) == "value2");
                };

                it("should set failbit on the stream if parsing fails (simulated by empty stream after some data)") = [] {
                    ini::ini_manager manager;
                    std::stringstream sstream;
